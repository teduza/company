...
export function initCosmicScene(container) {
  ...
  scene.fog = new THREE.FogExp2(0x0a0d15, 0.0008);
  ...
  const globeDetail = isMobile ? 4 : 6;
  const globeGeo = new THREE.IcosahedronGeometry(18, globeDetail);
  ...
  // Procedural displacement + coloring for terrain (deep navy basins, blue plains, pale highlands)
  const posAttr = globeGeo.attributes.position;
  const colorArr = new Float32Array(posAttr.count * 3);
  const deepBlue = new THREE.Color(0x0c2a63);
  const midBlue = new THREE.Color(0x2f6fed);
  const highBlue = new THREE.Color(0x8ec5ff);
  for (let i = 0; i < posAttr.count; i++) {
    ...
    const col = t < 0.5
      ? deepBlue.clone().lerp(midBlue, t * 2)
      : midBlue.clone().lerp(highBlue, (t - 0.5) * 2);
    const shade = 1 + fine * 0.12;
    colorArr[i * 3] = col.r * shade;
    colorArr[i * 3 + 1] = col.g * shade;
    colorArr[i * 3 + 2] = col.b * shade;
  }
  globeGeo.setAttribute('color', new THREE.BufferAttribute(colorArr, 3));
  ...
  const glowMat = new THREE.ShaderMaterial({
    uniforms: { glowColor: { value: new THREE.Color(0x5b9dff) } },
    vertexShader: `
  ...
  // Lighting
  const ambient = new THREE.AmbientLight(0x102244, 0.4);
  scene.add(ambient);
  const sun = new THREE.DirectionalLight(0x9fc4ff, 1.8);
  sun.position.set(50, 20, 40);
  scene.add(sun);
  const rim = new THREE.DirectionalLight(0x6a8fff, 0.3);
    ...
    const starMat = new THREE.ShaderMaterial({
      ...
      fragmentShader: `
        ...
        void main() {
          ...
          vec3 color = mix(vec3(0.92, 0.95, 1.0), vec3(0.7, 0.82, 1.0), vSize / 2.5);
  ...
  function animate() {
    ...
    camera.lookAt(globeGroup.position.x, globeGroup.position.y + scrollProgress * 10, globeGroup.position.z);

    // Globe moves away as we scroll
    globeGroup.position.z = scrollProgress * -150;
    globeGroup.position.y = scrollProgress * -20;
